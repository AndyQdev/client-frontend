 ...
 */
export const VendfyLogo = ({ variant = 'full', size = 40, className = '', dark = false }: { variant?: string; size?: number; className?: string; dark?: boolean }) => {
  // Icon only
  if (variant === 'icon') {
    const frameColor = dark ? '#ffffff' : '#0F172A';
    const doorInner = dark ? '#0F172A' : '#ECFDF5';
    const handleColor = dark ? '#ffffff' : '#0F172A';
    return (
      <svg
      ...
      >
        ...
        <rect x="8" y="34" width="92" height="16" rx="4" fill={frameColor} />
        <path d="M8 50 Q18 60, 28 50 Q38 60, 48 50 Q58 60, 68 50 Q78 60, 88 50 Q96 60, 100 50" fill={frameColor} />
        <rect x="38" y="62" width="32" height="44" rx="5" fill={frameColor} />
 ...
 */
export const VendfyIcon = ({ size = 32, className = '', dark = false }: { size?: number; className?: string; dark?: boolean }) => (
  <VendfyLogo variant="icon" size={size} className={className} dark={dark} />
);
